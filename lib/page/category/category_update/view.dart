...
/// 项目修改
class CategoryUpdatePage extends GetView<CategoryUpdateLogic> {
  final logic = Get.find<CategoryUpdateLogic>();

  CategoryUpdatePage({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return QScaffold(
      resizeToAvoidBottomInset: true,
      title: controller.details?.itemName ?? '',
      isCenterTitle: true,
      backFunction: () {
        showGetCommonDialog(
            content: QString.whetherToSaveTheCurrentEdit.tr ?? '',
            title: '',
            cancel: () => Get.back(),
            confirm: () {
      ...
      body: Column(
        children: [
          ...
          Expanded(child: _buildAttributes()),
      ...
      child: Row(
        children: [
          InkWell(
            ...
            child: GetBuilder<CategoryUpdateLogic>(
  ...
  _buildAttributes() {
    return GetBuilder<CategoryUpdateLogic>(
          ...
          return Container(
            ...
            child: Row(
              children: [
                ...
                SizedBox(width: QSize.space10),
                Expanded(
                    child: buildSecondTitle(
                        title: info.attributeHint ??
                            QString.categoryPleaseEnterLabelName.tr)),
                const Spacer(),
                (info.base == 1)
                    ? Container()
                    : IconButton(
  ...
  _buildSaveButton() {
    return Padding(
      ...
      child: QButtonRadius(
        ...
        callback: () => controller.updateProject(),
      ),
    );
  }
